...
for(let i=0; i<labels.length; i++) {
    // Spread them out in the center so they don't overlap too much
    let left = 30 + (i * 5);
    let top = 40 + (i * 5);
    batchHtml += `            <button class="mf-hotspot temp-drag" style="left: ${left}%; top: ${top}%; width: 5%; height: 5%; border-radius: 12px;"></button>\n`;
            ...
            <script>
              document.addEventListener('DOMContentLoaded', () => {
                ...
                document.getElementById('save-layout-btn').addEventListener('click', () => {
                  // We get all temp-drag elements (both hotspots and callouts) so we can update the backend properly
...
if (injectionPoint > -1) {
    ...
    console.log('Successfully injected batch 2 elements and tools.');
} else {

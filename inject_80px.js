...
for(let i=0; i<labels.length; i++) {
    let left = 30 + (i * 5);
    let top = 40 + (i * 5);
    batchHtml += `            <div class="mf-hotspot temp-drag" style="left: ${left}%; top: ${top}%; width: 160px; height: 160px; border-radius: 50%;"></div>\n`;
...
if (insertIdx > -1) {
    c = c.substring(0, insertIdx) + batchHtml + cleanDevToolsJS + '\n          ' + c.substring(insertIdx);
    fs.writeFileSync('index.html', c);
    console.log('Injected 80px radius circular borders for batch 2.');
}

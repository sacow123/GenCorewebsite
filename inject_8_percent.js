...
// The marker where we injected the previous border
const prevBorderEnd = c.indexOf('<!-- Developer Drag & Drop UI (Callouts Only) -->');

if (prevBorderEnd > -1) {
    let batchHtml = `\n            <!-- 배치 2: 8% 원형 테두리 -->\n`;
    batchHtml += `            <button class="mf-hotspot temp-drag" style="left: 30%; top: 40%; width: 8%; aspect-ratio: 1 / 1; height: auto; border-radius: 50%;"></button>\n`;
    batchHtml += `            <div class="mf-callout-custom center temp-drag" style="left: 30%; top: 40%; min-width: 250px;"><span class="title">정지 버튼</span><span class="desc">: 가공을 정지합니다</span></div>\n\n            `;
    
    // Replace the Callouts Only dev tools with the Clean Move Only dev tools
    const devToolsEnd = c.indexOf('</script>', prevBorderEnd) + 9;
...
`;
    ...
    c = c.substring(0, prevBorderEnd) + batchHtml + devToolsJS + c.substring(devToolsEnd);
    fs.writeFileSync('index.html', c);
    console.log('Injected 8% circular border and reset dev tools.');
}

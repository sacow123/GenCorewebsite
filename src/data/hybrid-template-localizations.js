...
(function () {
  ...
  const en = {
    ...
    'Overall finishing occlusal side (bridge)_[M1.0B_L15]': 'Overall finishing occlusal side (bridge)_[M1.0B_L15]',
    'Overall finishing cavity side (bridge)_[M1.0B_L15]': 'Overall finishing cavity side (bridge)_[M1.0B_L15]',
    '교합면을 1.0mm 공구를 사용해 전체적으로 정삭합니다.': 'Finishes the occlusal side overall using a 1.0 mm tool.',
    '계산: 해당 과정의 진행 여부를 선택하실 수 있습니다. (기본 설정값: 예)': 'Calculate: You can choose whether to run this process. (Default: Yes)',
    '일반 설정': 'General settings',
    '증분식 경계 옵셋': 'Incremental boundary offset',
    '증분식 경계 각도': 'Incremental boundary angle',
    "만약 계산중에 '경계 옵셋이나 경계각이 너무 적음'과 같은 에러가 발생한다면": 'If an error such as “Boundary offset or boundary angle is too small” occurs during calculation,',
    '해당 수치들을 조금 늘려 다시 시도해보세요.': 'increase these values slightly and try again.',
  ...
  const ja = {
    ...
    '조정 가능한 항목들': '調整可能な項目', 'Overall finishing occlusal side (bridge)_[M1.0B_L15]': '咬合面全体仕上げ（ブリッジ）_[M1.0B_L15]', 'Overall finishing cavity side (bridge)_[M1.0B_L15]': 'キャビティ側全体仕上げ（ブリッジ）_[M1.0B_L15]', '교합면을 1.0mm 공구를 사용해 전체적으로 정삭합니다.': '1.0 mmツールで咬合面全体を仕上げ加工します。', '계산: 해당 과정의 진행 여부를 선택하실 수 있습니다. (기본 설정값: 예)': '計算：この工程を実行するかどうかを選択できます。（初期設定：はい）', '일반 설정': '一般設定', '증분식 경계 옵셋': '増分境界オフセット', '증분식 경계 각도': '増分境界角度', "만약 계산중에 '경계 옵셋이나 경계각이 너무 적음'과 같은 에러가 발생한다면": '計算中に「境界オフセットまたは境界角度が小さすぎます」のようなエラーが発生した場合は、', '해당 수치들을 조금 늘려 다시 시도해보세요.': 'これらの値を少し上げて再試行してください。', '만약 계산중에 에러가 발생한다면 해당 수치들을 조금 늘려 다시 시도해보세요.': '計算中にエラーが発生した場合は、これらの値を少し上げて再試行してください。', '만약 계산중에 에러가 발생한다면 해당 수치들을 조금 늘려보세요.': '計算中にエラーが発生した場合は、これらの値を少し上げてください。',
  ...
  Object.assign(ja, {
    'Fissure machining [M1.0B_L15]': '裂溝加工 [M1.0B_L15]',
    'Fissure machining [Z0.6BX05]': '裂溝加工 [Z0.6BX05]'
  });

  Object.assign(en, {
    '소재: Wax disk': 'Material: Wax disk',
    '보철물 유형: Flexible denture': 'Prosthesis type: Flexible denture',
    '선택 사항': 'Optional',
    '해당 수치들을 조금 늘려 다시 시도해보세요': 'increase these values slightly and try again.',
    '교합면의 그루브를 0.3mm 공구를 사용해 가공합니다.': 'Machines occlusal grooves using a 0.3 mm tool.',
    '선택 사항 (Fissure machining [Z0.6BX05] 사용, 혹은 UDA CAT 3 or 4 사용 시)': 'Optional (when using Fissure machining [Z0.6BX05] or UDA category 3 or 4).',
    '선택 사항 (Fissure machining [Z0.3BX03] 사용, 혹은 UDA CAT 5사용 시)': 'Optional (when using Fissure machining [Z0.3BX03] or UDA category 5).'
  });
  Object.assign(ja, {
    '소재: Wax disk': '材料：ワックスディスク',
    '보철물 유형: Flexible denture': '補綴物の種類：フレキシブルデンチャー',
    '선택 사항': '任意',
    '해당 수치들을 조금 늘려 다시 시도해보세요': 'これらの値を少し上げて再試行してください。',
    'Fissure machining [Z1.0BX15]': '裂溝加工 [Z1.0BX15]',
    'Fissure machining [Z0.6BX05]': '裂溝加工 [Z0.6BX05]',
    'Fissure machining [Z0.3BX03]': '裂溝加工 [Z0.3BX03]',
    '교합면의 그루브를 0.3mm 공구를 사용해 가공합니다.': '0.3 mmツールで咬合面のグルーブを加工します。',
    '선택 사항 (Fissure machining [Z0.6BX05] 사용, 혹은 UDA CAT 3 or 4 사용 시)': '任意（裂溝加工 [Z0.6BX05] または UDAカテゴリ3・4を使用する場合）。',
    '선택 사항 (Fissure machining [Z0.3BX03] 사용, 혹은 UDA CAT 5사용 시)': '任意（裂溝加工 [Z0.3BX03] または UDAカテゴリ5を使用する場合）。',
    'Category 1 : T39M1.0B': 'カテゴリ1：T39M1.0B',
    'Category 2 : T39M1.0B': 'カテゴリ2：T39M1.0B',
    'Category 3 : T39M1.0B→T33-Z0.6B': 'カテゴリ3：T39M1.0B→T33-Z0.6B',
    'Category 4 : T39M1.0B→T33-Z0.6B': 'カテゴリ4：T39M1.0B→T33-Z0.6B',
    'Category 5 : T32-Z1.0B→T33-Z0.6B→T34Z0.3B': 'カテゴリ5：T32-Z1.0B→T33-Z0.6B→T34Z0.3B',
    'Category 6 : T32-Z1.0B→T33-Z0.6B→T34Z0.3B': 'カテゴリ6：T32-Z1.0B→T33-Z0.6B→T34Z0.3B',
    'Category 1 : T32-Z1.0B': 'カテゴリ1：T32-Z1.0B',
    'Category 2 : T32-Z1.0B': 'カテゴリ2：T32-Z1.0B',
    'Category 3 : T32-Z1.0B→T33-Z0.6B': 'カテゴリ3：T32-Z1.0B→T33-Z0.6B',
    'Category 4 : T32-Z1.0B→T33-Z0.6B': 'カテゴリ4：T32-Z1.0B→T33-Z0.6B'
  });

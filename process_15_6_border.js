...
const rawData = [
  {"index":0,"class":"mf-hotspot","style":"left: 29.3%; top: 39.2%; width: 15.6%; aspect-ratio: 1 / 1; height: auto; border-radius: 50%; cursor: move; pointer-events: auto; right: auto;"},
  {"index":1,"class":"mf-callout-custom center","style":"left: 30%; top: 40%; min-width: 250px; cursor: move; pointer-events: auto;"}
];

let hsStyle = rawData[0].style
    .replace(/cursor: move;\s*/g, '')
    .replace(/pointer-events: auto;\s*/g, '')
    .replace(/right: auto;\s*/g, '')
    .trim();

let coStyle = rawData[1].style
    .replace(/cursor: move;\s*/g, '')
    .replace(/pointer-events: auto;\s*/g, '')
    .trim();
...
// Replace the border html
let batchHtml = `\n            <!-- 배치 2: 15.6% 원형 테두리 (재생 버튼) -->\n`;
batchHtml += `            <button class="mf-hotspot" style="${hsStyle}"></button>\n`;
batchHtml += `            <div class="mf-callout-custom center temp-drag" style="${coStyle}"><span class="title">재생 버튼</span><span class="desc">: 가공을 시작합니다</span></div>\n\n            `;

const markerStart = c.indexOf('<!-- 배치 2: 15.6% 원형 테두리 -->');
if (markerStart > -1) {
    const markerEnd = c.indexOf('<!-- Developer Drag & Drop UI (Clean Move Only) -->', markerStart);
    if (markerEnd > -1) {
        c = c.substring(0, markerStart) + batchHtml + c.substring(markerEnd);
    }
}

// Now replace the Dev Tools Script to only allow dragging the new callout
...
const calloutDragJS = `
            ...
            <script>
              document.addEventListener('DOMContentLoaded', () => {
                ...
                // Only target the specific temp-drag callout
...
if (devToolsStart > -1 && devToolsEnd > -1) {
    ...
    console.log('Processed 15.6% border coordinates and prepared callout dragging.');
} else {
